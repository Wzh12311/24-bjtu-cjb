...
import com.face.server.DetectLiveFace;
...
public class FaceServiceImpl extends ServiceImpl<FaceMapper, Face>
    ...
    @Autowired
    DetectLiveFace detectLiveFace1;
    ...
    public FaceResult vef(String imageBase) {
        ...
        if (faceList.size() == 0){
            ...
            for (Face face : faceList) {
                FaceResult liveFace = detectLiveFace1.detectLiveFace(imageBase);
                FaceResult faceResult = faceContrastServer.faceContrast(face.getFaceBase(), imageBase);
                //是否是活体
                if (liveFace.getCode() == FaceResult.LIVE_FACE && liveFace.getScore() > 80) {
                    // 是否比对成功
                    ...
                    return faceState != null?faceState:FaceResult.error(FaceResult.NOT_FOUND_FACE,"摄像头中为静态图片，禁止登录",faceResult.getScore());
                }
